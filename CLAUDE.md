...
| `apps-script/classicals-proxy.gs` | Optional Google Apps Script — scrapes classicals.de solo-piano selection (server-side, bypasses 403/CORS) and returns each MP3 as base64; caches downloads in a `radio-music-cache` Drive folder. Powers Radio interstitial music. | — |
...
| `tracker-radio.js` | Tracker Radio tab — on-demand AI "radio station". Two-phase per episode: (1) Claude (same browser SDK/key as Chat) writes a segmented DJ script per channel/persona → `draft`; user reads it on the **review screen**, then (2) ElevenLabs TTS (`api.elevenlabs.io`, model `eleven_flash_v2_5`, MP3) voices each segment → `ready`. **Manual (no-API-key) path:** each channel card also has **📋 Prompt** (`openPromptModal` → `buildCopyPrompt`, one self-contained prompt requesting the whole show in `## N. Title` Markdown) to copy into claude.ai, and **📥 Paste** (`openPasteModal` → `parsePastedScript`, the inverse split, falls back to `chunkIntoSegments` when headings are absent) to import the reply straight into a `draft` episode on the review screen — so phase 1 works without a Claude key. Voices are fetched live from the account (`GET /v1/voices`, cached in `health:radio:elevenVoices`) and user-selectable per channel/episode with a free `preview_url` sample; API key in `health:elevenLabsKey`. Script text + audio Blobs both in IndexedDB (`health-radio` DB, store `radioAudio`; keys `epId:idx` audio, `epId:idx:txt` script). Episode status: `scripting`→`draft`→`synthesizing`→`ready`. Progressive + resumable + cancelable; custom `<audio>` player with pause/seek/±15s/segment nav + resume + a Script transcript. Exposes `window.renderRadio()`. **Interstitial music**: between every spoken segment (not after the last) it plays a random 3–5 min slice of a random royalty-free piano piece with fade in/out, showing the piece title. Tracks come from `apps-script/classicals-proxy.gs` (web-app URL in `health:radio:musicProxyUrl`, set in Setup → Radio station); the scraped list is cached in `health:radio:musicTracks` (weekly refresh) and each decoded MP3 Blob in the same IndexedDB store under key `music:<id>`. Always on when a proxy URL is set; no-op otherwise. A second `<audio id="rp-music">` element + `activeAudio()` route the transport controls during a break. | — |
