...
1. Create the secret (the upstream Modal-Key/Modal-Secret come from the auto
   ...
   ScadPad's Chat settings):
     ...
     modal secret create llm-proxy-secrets \\
...
3. Deploy:
     ...
     modal deploy modal/llm_proxy.py
...
@app.function(image=image, secrets=[modal.Secret.from_name("llm-proxy-secrets")])
@modal.asgi_app()
def fastapi_app():
    return build_app()
