...
- [x] **No tests.** Minimal harness added under `tests/` (Node's built-in `node:test`, run with `npm test`). A `vm` sandbox loads the pure-logic files with browser stubs; 27 tests cover `calcSGTax`, `getOngoingDueInfo` (recurring date math), `mergeData`, `mergeHistoryData`, `calcCpfProjection`, and `calcRetirementPlan`. See `tests/README.md`.
